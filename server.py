...
@app.get("/api/coins")
def api_coins_alias():
    """UI coin dropdown compatibility.

    Frontend farklı yerlerde farklı shape bekleyebiliyor.
    Bu endpoint her iki formatı da sağlar:
      - coins: [{symbol, label, value}]
      - data:  [{value, label}]
      - symbols: ["BTC/USDT", ...]

    Query:
      exchange=OKX|BINANCE|BYBIT|GATEIO (opsiyonel)
    """
    exch = safe_str(request.args.get("exchange") or "OKX").strip().upper() or "OKX"

    payload = None
    try:
        # önce public coins cache'den üret
        if exch and exch != "OKX":
            payload = _public_coins_cached_by_exchange(exch)
        else:
            payload = _public_coins_cached()
    except Exception:
        payload = None

    # fallback: mevcut eski endpointler
    if not payload:
        try:
            r = api_public_coins()
    ...
    if isinstance(payload, dict):
        coins = payload.get("coins") or payload.get("data") or []
    ...
    if isinstance(coins, list):
        for it in coins:
            if isinstance(it, str):
                ...
                sym = it.get("symbol") or it.get("value") or it.get("name")
            else:
                sym = None
            sym = safe_str(sym or "").strip().upper()
            if not sym:
                continue
    ...
    # UI-friendly formats
    data = [{"value": s, "label": s} for s in symbols]
    coins2 = [{"symbol": s, "value": s, "label": s} for s in symbols]
    ...
    if isinstance(payload, dict):
        ...
        if "cached" in payload:
            out["cached"] = payload.get("cached")

    return jsonify(out)

@app.get("/api/ohlc")
def api_ohlc_alias():
    # UI /api/ohlc çağırıyor
    try:
...
def api_public_positions_alias():
    # UI login varsa pozisyonları ister. Login yoksa boş döner.
    try:
    ...
    # /api/positions ile aynı çıktı formatını verelim
    try:
...
# AI execute: UI "İşlem Yap" popup'ı bunu çağırır
@app.post("/api/ai/execute")
def api_ai_execute():
    """Execute a DEMO trade via AI action.

    Body: {"user_id":"...","symbol":"BTC/USDT","side":"BUY|SELL","usdt_amount":20}
    - BUY: demo BUY
    - SELL: demo SELL (sell all open for symbol)
    """
    j = request.get_json(silent=True) or {}
...
def api_chat_rooms():
    # Basit sabit odalar
    rooms = [
        {"id": "USDTTRY", "title": "USDT/TRY Trading"},
        {"id": "BTCTRY", "title": "BTC/TRY Trading"},
        {"id": "ETHTRY", "title": "ETH/TRY Trading"},
        {"id": "GENERAL", "title": "Genel Tartisma"},
    ]
...
@app.get("/api/chat")
def api_chat_get():
    # UI bazen login cookie gondermeyebilir. Login yoksa da bos liste donelim.
    ...
    try:
        rows = conn.execute(
            "SELECT id, username, message, created_at FROM chat_messages WHERE id>? ORDER BY id ASC LIMIT 200",
            (since_id,),
        ).fetchall()
...
@app.post("/api/chat")
def api_chat_post():
    # UI bazen login cookie gondermeyebilir. Login yoksa query/body ile username al.
    username = safe_str(session.get("username") or "")
    j = request.get_json(silent=True) or {}
    if not username:
        username = safe_str(j.get("username") or request.args.get("username") or "guest")
...
# Alias: UI bazı yerlerde /api/coins kullanıyor
@app.get("/api/coins")
def api_coins_alias():
    try:
...
def _demo_buy(uid: str, symbol: str, amount_usdt: float, plan: str = ""):
    ...
    with DEMO_LOCK:
        ...
        if trade_limit is not None and trade_count >= int(trade_limit):
            return {"ok": False, "error": "Paket işlem limiti doldu"}

        # Coin limiti: açık pozisyonlarda kaç farklı symbol var
        open_syms = set()
...
<body>
  <div class=\"wrap\">
    <div class=\"top\">
      <div>
        <div class=\"brand\">Atalay Ulusoy Auto Trade • Admin</div>
        <div class=\"muted\">Backend panel • UI dosyalarina dokunmadan debug ve kontrol</div>
      </div>
    ...
    <div class=\"grid\">
      ...
      <div class=\"card span6\">
        ...
        <div class=\"row\">
          ...
          <span class=\"muted\">/api/public/positions • /api/trades</span>
        </div>
        <div class=\"sep\"></div>
        <pre id=\"ptOut\">\n</pre>
      </div>
...
<script>
  ...
  async function sendChat(){
    ...
    try{
      const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({message:msg})})
      const data=await r.json().catch(()=>({raw:'no json'}))
