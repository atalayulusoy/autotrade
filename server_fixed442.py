...
# =========================
# Panel context (LIVE vs TEST)
# =========================
# IMPORTANT:
# - LIVE and /test must never mix
# - We do NOT rely on session only (tabs share cookies).
# - Primary signal: request.path + request.referrer.
def _infer_panel_ctx() -> str:
    try:
        p = (request.path or "")
        ref = (request.referrer or "")
        if p.startswith("/test") or "/test" in ref:
            return "test"
        return "live"
    except Exception:
        return "live"
...
def _panel_ctx() -> str:
    # Prefer per-request inference; fall back to last known session value
    ctx = _infer_panel_ctx()
    if ctx in ("live", "test"):
        return ctx
    try:
        return session.get("_panel_ctx", "live")
    except Exception:
        return "live"

def _set_panel_ctx(ctx: str) -> None:
    # Keep for compatibility, but do not trust it as sole signal
    try:
        if ctx in ("live", "test"):
            session["_panel_ctx"] = ctx
    except Exception:
        pass

# Backward-compat alias (some older code paths may call this)
def set_panel_ctx(ctx: str) -> None:
    _set_panel_ctx(ctx)

def _is_test_ctx() -> bool:
    return _panel_ctx() == "test"
...
def _all_errors(e):
    ...
    return Response(
        "<h1>500</h1><pre style='white-space:pre-wrap;font-size:12px'>"
        + tb +
        "</pre>",
        status=500,
        mimetype="text/html"
...
def redirect_back(fallback: str = "/"):
    """POST işlemlerinden sonra kullanıcının geldiği sayfaya geri dön.
    Test panelinde işlem yapınca ana panele atma problemini çözer.
    """
    try:
        ref = request.referrer or ""
        if ref:
            ...
            parts = urlsplit(ref)
            # Sadece aynı host'a veya relative path'e izin ver
            if parts.netloc:
...
def api_positions():
    ...
    username = session.get("username")
    ctx = _panel_ctx()
    # TEST panelinde açık pozisyonlar paper_test_positions üzerinden gelir (izole)
    if ctx == "test":
        try:
            out = []
            rows = paper_test_list_positions(username)
            for r in rows:
                ex = safe_str(r.get("exchange") or "OKX").upper()
                sym = canon_symbol(r.get("symbol") or "")
                qty = float(r.get("qty") or 0.0)
                entry_px = float(r.get("entry_px") or 0.0)
                invested = float(r.get("invested_usdt") or 0.0)
                cur_px = 0.0
                try:
                    ...
                    cur_px = 0.0
                pnl = None
                if cur_px > 0 and qty > 0 and entry_px > 0:
                    pnl = (cur_px - entry_px) * qty
                out.append({
                    ...
                    "entry_usdt": invested,
                    "buy_fee_usdt": float(r.get("fee_buy") or 0.0),
                    "dry_run": 1,
                    "created_at": safe_str(r.get("entry_ts") or ""),
...
# =========================
# Market data (Charts page)
# =========================
def _instid_okx(symbol: str) -> str:
...
def _market_ticker_okx(symbol: str) -> dict:
    import requests
    inst = _instid_okx(symbol)
    r = requests.get("https://www.okx.com/api/v5/market/ticker", params={"instId": inst}, timeout=8)
    j = r.json()
    ...
    return {
        ...
        "low24h": f("low24h"),
        "vol24h": f("vol24h"),
        "ts": int(float(data.get("ts") or 0.0)/1000) if data.get("ts") else now_ts(),
    }

def _market_candles_okx(symbol: str, bar: str = "1m", limit: int = 200) -> list:
    import requests
    inst = _instid_okx(symbol)
    lim = int(limit or 200)
    if lim < 10: lim = 10
    if lim > 300: lim = 300
    r = requests.get("https://www.okx.com/api/v5/market/candles", params={"instId": inst, "bar": bar, "limit": str(lim)}, timeout=8)
    j = r.json()
    data = j.get("data") or []
    out = []
...
def api_market_ticker():
    ...
    if rr:
        return rr
    ex = safe_str(request.args.get("exchange") or "OKX").strip().upper()
    sym = canon_symbol(request.args.get("symbol") or DEFAULT_SYMBOL)
    if ex not in ("OKX",):
        ex = "OKX"
    try:
        t = _market_ticker_okx(sym)
    except Exception as e:
        return jsonify({"ok": False, "err": safe_str(e)})
    usdt_try = float(_get_usdt_try_rate() or 0.0)
    out = dict(t)
    out["symbol"] = sym
    out["exchange"] = ex
    out["usdt_try"] = usdt_try
    if usdt_try > 0:
        out["last_tl"] = out["last"] * usdt_try
        out["bid_tl"] = out["bid"] * usdt_try
        out["ask_tl"] = out["ask"] * usdt_try
        out["high24h_tl"] = out["high24h"] * usdt_try
        out["low24h_tl"] = out["low24h"] * usdt_try
    return jsonify({"ok": True, "ticker": out})
...
def api_market_candles():
    ...
    ex = safe_str(request.args.get("exchange") or "OKX").strip().upper()
    sym = canon_symbol(request.args.get("symbol") or DEFAULT_SYMBOL)
    bar = safe_str(request.args.get("bar") or "1m").strip()
    limit = int(safe_str(request.args.get("limit") or "200") or 200)
    if ex not in ("OKX",):
        ex = "OKX"
    try:
        data = _market_candles_okx(sym, bar=bar, limit=limit)
        return jsonify({"ok": True, "exchange": ex, "symbol": sym, "bar": bar, "candles": data})
    except Exception as e:
        return jsonify({"ok": False, "err": safe_str(e), "candles": []})
# =========================
# BINANCE PUBLIC MARKET API (Grafikler)
# =========================

_BINANCE_CACHE = {"symbols": {"ts": 0, "data": []}}

def _binance_get(url: str, params: dict = None, timeout: float = 6.0):
    try:
        r = requests.get(url, params=params or {}, timeout=timeout, headers={"User-Agent": "AU-AutoTrade"})
        r.raise_for_status()
        return r.json()
    except Exception:
        return None

def _binance_symbols_cached() -> List[str]:
    now = time.time()
    if _BINANCE_CACHE["symbols"]["data"] and (now - _BINANCE_CACHE["symbols"]["ts"] < 300):
        return _BINANCE_CACHE["symbols"]["data"]
    j = _binance_get("https://api.binance.com/api/v3/exchangeInfo", timeout=8.0)
    out = []
    try:
        for s in (j or {}).get("symbols", []):
        ...
        pass
    out = sorted(list({x for x in out if x}))
    _BINANCE_CACHE["symbols"]["ts"] = now
    _BINANCE_CACHE["symbols"]["data"] = out
    return out

@app.get("/api/binance/symbols")
def api_binance_symbols():
    rr = require_login()
    if rr:
        return rr
    return jsonify({"ok": True, "symbols": _binance_symbols_cached()})

@app.get("/api/binance/mini")
def api_binance_mini():
    rr = require_login()
    if rr:
        return rr
    sym = safe_str(request.args.get("symbol") or "BTCUSDT").upper().strip()
    j = _binance_get("https://api.binance.com/api/v3/ticker/price", params={"symbol": sym}, timeout=5.0)
    last = safe_float((j or {}).get("price"), 0.0)
    return jsonify({"ok": True, "symbol": sym, "last": last})
...
def api_binance_summary():
    ...
    t24 = _binance_get("https://api.binance.com/api/v3/ticker/24hr", params={"symbol": sym}, timeout=6.0) or {}
    book = _binance_get("https://api.binance.com/api/v3/ticker/bookTicker", params={"symbol": sym}, timeout=6.0) or {}

    out = {
        "symbol": sym,
        "last": safe_float(t24.get("lastPrice"), 0.0),
        "bid": safe_float(book.get("bidPrice"), 0.0),
        "ask": safe_float(book.get("askPrice"), 0.0),
        "changePercent": safe_float(t24.get("priceChangePercent"), 0.0),
        "high": safe_float(t24.get("highPrice"), 0.0),
        "low": safe_float(t24.get("lowPrice"), 0.0),
        "avg": safe_float(t24.get("weightedAvgPrice"), 0.0),
        "volume": safe_float(t24.get("volume"), 0.0),
        "quoteVolume": safe_float(t24.get("quoteVolume"), 0.0),
    }
...
def api_binance_klines():
    ...
    if rr:
        return rr
    sym = safe_str(request.args.get("symbol") or "BTCUSDT").upper().strip()
    interval = safe_str(request.args.get("interval") or "1m").strip()
    limit = int(safe_int(request.args.get("limit") or 600, 600))
    limit = max(10, min(1000, limit))

    j = _binance_get("https://api.binance.com/api/v3/klines", params={"symbol": sym, "interval": interval, "limit": limit}, timeout=8.0)
    candles = []
    try:
        for it in (j or []):
            ...
            candles.append({
                "time": t,
                "open": safe_float(it[1], 0.0),
                "high": safe_float(it[2], 0.0),
                "low": safe_float(it[3], 0.0),
                "close": safe_float(it[4], 0.0),
            })
    except Exception:
        candles = []
    return jsonify({"ok": True, "symbol": sym, "interval": interval, "candles": candles})

@app.get("/api/binance/depth")
def api_binance_depth():
    rr = require_login()
    if rr:
        return rr
    sym = safe_str(request.args.get("symbol") or "BTCUSDT").upper().strip()
    limit = int(safe_int(request.args.get("limit") or 15, 15))
    limit = max(5, min(50, limit))
    j = _binance_get("https://api.binance.com/api/v3/depth", params={"symbol": sym, "limit": limit}, timeout=6.0) or {}
    bids = j.get("bids") or []
    asks = j.get("asks") or []
    return jsonify({"ok": True, "symbol": sym, "bids": bids, "asks": asks})
...
def api_binance_trades():
    ...
    if rr:
        return rr
    sym = safe_str(request.args.get("symbol") or "BTCUSDT").upper().strip()
    limit = int(safe_int(request.args.get("limit") or 18, 18))
    limit = max(5, min(50, limit))
    j = _binance_get("https://api.binance.com/api/v3/aggTrades", params={"symbol": sym, "limit": limit}, timeout=6.0) or []
    out = []
    try:
        for it in j:
            out.append({
                "time": safe_int(it.get("T"), 0),
                "price": safe_float(it.get("p"), 0.0),
                "qty": safe_float(it.get("q"), 0.0),
                "isBuyerMaker": bool(it.get("m")),
            })
    except Exception:
        out = []
    # newest last feels nicer in table
    out = sorted(out, key=lambda x: x.get("time", 0), reverse=True)
    return jsonify({"ok": True, "symbol": sym, "trades": out})
...
def auto_list(username: str) -> List[Dict[str, Any]]:
    """Auto kurallarını döndürür.
    - Live: auto_rules
    - Test: paper_test_autorules (izole)
    """
    ...
    try:
        if ctx == "test":
            rows = conn.execute(
                "SELECT id, username, exchange_id, symbol, usdt_amount, use_all_balance, enabled, created_at "
                "FROM paper_test_autorules WHERE username=? ORDER BY id DESC",
                (username,),
            ).fetchall()
        ...
        rows = conn.execute(
            ...
            "FROM auto_rules WHERE username=? ORDER BY id DESC",
            (username,),
        ).fetchall()
        return [dict(r) for r in rows]
    finally:
...
def ensure_paper_test_schema(conn):
    ...
    # PATCH: backward/forward compatible columns for paper_test_positions
    for col, decl in [
            ("exchange_id", "TEXT"),
            ("created_at", "INTEGER"),
            ("opened_at", "INTEGER"),
        ...
        _ensure_column(cur, "paper_test_positions", col, decl)
    ...
    # PATCH: backward/forward compatible columns for paper_test_trades
    for col, decl in [
            ("exchange_id", "TEXT"),
            ("entry_price", "REAL"),
            ("exit_price", "REAL"),
        ...
        _ensure_column(cur, "paper_test_trades", col, decl)
...
# =========================
# Paper TEST trade execution (isolated, NO real order)
# =========================
PAPER_FEE_RATE = 0.001  # 0.10% each side (sim). Can be adjusted later.

# =========================
# Paper TEST AUTO engine (izole)
# =========================
_PAPER_AUTO_LOCK = threading.Lock()
_PAPER_AUTO_PRICE = {}  # key -> deque of recent prices

def _paper_auto_price_key(username: str, exchange_id: str, symbol: str) -> str:
    return f"{safe_str(username)}|{safe_str(exchange_id).upper()}|{canon_symbol(symbol)}"

def _paper_auto_push_price(key: str, price: float, maxlen: int = 90) -> None:
    try:
        from collections import deque
        with _PAPER_AUTO_LOCK:
            dq = _PAPER_AUTO_PRICE.get(key)
            if dq is None:
                dq = deque(maxlen=maxlen)
                _PAPER_AUTO_PRICE[key] = dq
            dq.append(float(price))
    except Exception:
        pass

def _paper_auto_get_window(key: str):
    with _PAPER_AUTO_LOCK:
        dq = _PAPER_AUTO_PRICE.get(key)
        if not dq:
            return []
...
def _paper_auto_should_buy(prices: list) -> bool:
    # Basit dip yakalama: son pencere içinde %0.30+ geri çekilme ve şimdi pencere dibine yakın
    ...
    drop = (mx - cur) / mx
    near_low = (cur - mn) / mn
    return (drop >= 0.003) and (near_low <= 0.002)

def _paper_auto_take_profit_price(entry_price: float) -> float:
    # Net %0.50 hedef (fee sim dahil kaba): 0.50% + iki taraf fee
    try:
        return float(entry_price) * (1.0 + 0.005 + (2.0 * float(PAPER_FEE_RATE or 0.0)))
    except Exception:
        return float(entry_price) * 1.006

def _paper_auto_loop():
    # Sürekli: enabled paper_test_autorules kontrol, fiyat çek, BUY/SELL karar ver
    while True:
        try:
            time.sleep(5)
            conn = db()
            try:
                # enabled rules
                rules = conn.execute(
                    "SELECT id, username, exchange_id, symbol, usdt_amount, use_all_balance, enabled "
                    "FROM paper_test_autorules WHERE enabled=1"
                ).fetchall()
                for r in rules:
                    try:
                        rr = dict(r)
                        username = safe_str(rr.get("username") or "")
                        ex = safe_str(rr.get("exchange_id") or "OKX").upper() or "OKX"
                        sym = canon_symbol(safe_str(rr.get("symbol") or ""))
                        if not username or not sym:
                            continue

                        # price
                        try:
                            px = float(public_price(ex, sym) or 0.0)
                        except Exception:
                            px = 0.0
                        if px <= 0:
                            continue

                        k = _paper_auto_price_key(username, ex, sym)
                        _paper_auto_push_price(k, px)

                        # açık pozisyon var mı
                        prow = conn.execute(
                            "SELECT id, entry_price, entry_px, entry_usdt, invested_usdt, qty, buy_fee_usdt, fee_buy "
                            "FROM paper_test_positions WHERE username=? AND UPPER(symbol)=UPPER(?) AND (status IS NULL OR status='OPEN') "
                        ...
                        if prow:
                            pp = dict(prow)
                            entry_p = float(pp.get("entry_price") or pp.get("entry_px") or 0.0)
                            tp = _paper_auto_take_profit_price(entry_p) if entry_p > 0 else 0.0
                            if tp > 0 and px >= tp:
                                # SELL
                                paper_test_exec_order(username, "SELL", ex, sym, 0.0, all_in=False)
                            continue

                        # pozisyon yoksa BUY kararı
                        win = _paper_auto_get_window(k)
                        if not _paper_auto_should_buy(win):
                            continue

                        all_in = int(rr.get("use_all_balance") or 0) == 1
                        usdt_amt = float(rr.get("usdt_amount") or 0.0)
                        paper_test_exec_order(username, "BUY", ex, sym, usdt_amt, all_in=all_in)
...
def _paper_now_ts() -> str:
    try:
        return datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=3))).strftime("%Y-%m-%d %H:%M:%S")
    except Exception:
        try:
...
def paper_test_exec_order(username: str, action: str, exchange_id: str, symbol: str, usdt: float, all_in: bool=False) -> Dict[str, Any]:
    """Paper TEST BUY/SELL (tam izole).
    - Gerçek emir YOK
    - open_positions / trades tablolarına dokunmaz
    - paper_test_state / paper_test_positions / paper_test_trades kullanır
    """
    username = safe_str(username).strip()
    action = safe_str(action).strip().upper()
    exchange_id = safe_str(exchange_id).strip().upper() or "OKX"
    symbol = canon_symbol(symbol)

    # state garanti
    st = paper_test_ensure_state(username) or {}
    conn = db()
    try:
        # anlık fiyat
        try:
            price = float(public_price(exchange_id, symbol) or 0.0)
        except Exception:
            price = 0.0
        if price <= 0:
            return {"ok": False, "err": "Fiyat alınamadı"}

        ts_txt = _paper_now_ts()
        nowi = now_ts()
        fee_rate = float(PAPER_FEE_RATE or 0.0)

        # helper: pozisyon satır kolonları
        cur = conn.cursor()
        cols_pos = {r[1] for r in cur.execute("PRAGMA table_info(paper_test_positions)").fetchall()}
        cols_tr  = {r[1] for r in cur.execute("PRAGMA table_info(paper_test_trades)").fetchall()}

        def _get_bal_usdt() -> float:
            # paper_test_state'de usdt kolonu farklı olabilir, garantili oku
            row = conn.execute("SELECT usdt FROM paper_test_state WHERE username=?", (username,)).fetchone()
            try:
        ...
        if action == "BUY":
            ...
            invest = float(usdt or 0.0)
            if all_in or invest <= 0:
                invest = float(bal or 0.0)

            if invest <= 0:
                return {"ok": False, "err": "Bakiye yetersiz"}

            # basit fee sim
            fee_buy = max(0.0, invest * fee_rate)
            net_invest = max(0.0, invest - fee_buy)
            qty = (net_invest / price) if price > 0 else 0.0
            if qty <= 0:
                return {"ok": False, "err": "Miktar çok küçük"}
            ...
            _add("exchange_id", exchange_id)
            _add("exchange", exchange_id)
            _add("symbol", symbol)
            _add("qty", float(qty))
            _add("entry_price", float(price))
            _add("entry_px", float(price))
            _add("entry_usdt", float(invest))
            _add("invested_usdt", float(invest))
            _add("buy_fee_usdt", float(fee_buy))
            _add("fee_buy", float(fee_buy))
            _add("fee_sell", 0.0)
            _add("fee_sell_usdt", 0.0)
            _add("opened_at", int(nowi))
            _add("created_at", int(nowi))
            _add("entry_ts", ts_txt)
            _add("status", "OPEN")

            q = f"INSERT INTO paper_test_positions({','.join(insert_cols)}) VALUES ({','.join(['?']*len(insert_cols))})"
            cur.execute(q, tuple(insert_vals))

            # state update
            new_bal = max(0.0, bal - invest)
            # paper_test_state yeni şema: usdt + updated_ts + updated_at vs (hepsini doldur)
            try:
                if "updated_ts" in {r[1] for r in cur.execute("PRAGMA table_info(paper_test_state)").fetchall()}:
                    cur.execute("UPDATE paper_test_state SET usdt=?, updated_ts=? WHERE username=?", (new_bal, ts_txt, username))
                else:
                    cur.execute("UPDATE paper_test_state SET usdt=?, updated_at=? WHERE username=?", (new_bal, nowi, username))
            except Exception:
                # en azından usdt güncelle
                try:
                    cur.execute("UPDATE paper_test_state SET usdt=? WHERE username=?", (new_bal, username))
                except Exception:
                    pass

            conn.commit()
            return {"ok": True, "action": "BUY", "exchange_id": exchange_id, "symbol": symbol, "price": price, "qty": qty, "invested_usdt": invest, "fee_buy_usdt": fee_buy}

        if action == "SELL":
            # en yeni OPEN pozisyonu bul (symbol bazlı)
            row = conn.execute(
                "SELECT * FROM paper_test_positions WHERE username=? AND UPPER(symbol)=UPPER(?) AND (status IS NULL OR status='OPEN') ORDER BY id DESC LIMIT 1",
                (username, symbol),
            ).fetchone()
            if not row:
                return {"ok": False, "err": "Açık pozisyon yok"}

            p = dict(row)
            pid = int(p.get("id") or 0)
            ex_db = safe_str(p.get("exchange_id") or p.get("exchange") or exchange_id).upper() or exchange_id
            qty = float(p.get("qty") or 0.0)
            entry_price = float(p.get("entry_price") or p.get("entry_px") or 0.0)
            entry_usdt = float(p.get("entry_usdt") or p.get("invested_usdt") or 0.0)
            fee_buy = float(p.get("buy_fee_usdt") or p.get("fee_buy") or 0.0)

            if qty <= 0 or entry_usdt <= 0 or entry_price <= 0:
                return {"ok": False, "err": "Pozisyon verisi bozuk"}

            proceeds = qty * price
            fee_sell = max(0.0, proceeds * fee_rate)
            pnl_net = (proceeds - fee_sell) - (entry_usdt)  # entry_usdt zaten buy_fee dahil değilse dahi kullanıcı toplam yatırım olarak entry_usdt esas
            # fee'yi ayrı tutuyoruz; net pnl = (proceeds - sellfee) - invested - buyfee
            pnl_net = (proceeds - fee_sell) - (entry_usdt + fee_buy)
            pnl_pct = (pnl_net / (entry_usdt + fee_buy)) if (entry_usdt + fee_buy) > 0 else 0.0

            # pozisyonu kapat: status + fee_sell
            try:
                if "fee_sell" in cols_pos:
                    cur.execute("UPDATE paper_test_positions SET fee_sell=?, status='CLOSED' WHERE id=? AND username=?", (float(fee_sell), pid, username))
                elif "fee_sell_usdt" in cols_pos:
                    cur.execute("UPDATE paper_test_positions SET fee_sell_usdt=?, status='CLOSED' WHERE id=? AND username=?", (float(fee_sell), pid, username))
                else:
                    cur.execute("DELETE FROM paper_test_positions WHERE id=? AND username=?", (pid, username))
            except Exception:
                try:
                    cur.execute("DELETE FROM paper_test_positions WHERE id=? AND username=?", (pid, username))
                except Exception:
            ...
            # trade kaydı (mevcut kolonlara göre)
            insert_cols = []
            insert_vals = []
            def _addt(c, v):
                if c in cols_tr:
            ...
            _addt("username", username)
            _addt("exchange_id", ex_db)
            _addt("exchange", ex_db)
            _addt("symbol", symbol)
            _addt("entry_price", entry_price)
            _addt("entry_px", entry_price)
            _addt("exit_price", price)
            _addt("exit_px", price)
            _addt("qty", qty)
            _addt("invested_usdt", entry_usdt)
            _addt("received_usdt", proceeds)
            _addt("fee_buy", fee_buy)
            _addt("buy_fee_usdt", fee_buy)
            _addt("fee_sell", fee_sell)
            _addt("fee_sell_usdt", fee_sell)
            _addt("fee_total_usdt", float(fee_buy + fee_sell))
            _addt("pnl_net_usdt", float(pnl_net))
            _addt("pnl_pct", float(pnl_pct))
            _addt("opened_at", int(p.get("opened_at") or p.get("created_at") or 0))
            _addt("closed_at", int(nowi))
            _addt("ts_open", safe_str(p.get("entry_ts") or p.get("ts_open") or ""))
            _addt("ts_close", ts_txt)
            _addt("created_at", int(nowi))

            if insert_cols:
                q = f"INSERT INTO paper_test_trades({','.join(insert_cols)}) VALUES ({','.join(['?']*len(insert_cols))})"
                cur.execute(q, tuple(insert_vals))

            # state: usdt += proceeds - sellfee
            bal = _get_bal_usdt()
            new_bal = float(bal + (proceeds - fee_sell))
            try:
                if "updated_ts" in {r[1] for r in cur.execute("PRAGMA table_info(paper_test_state)").fetchall()}:
                    cur.execute("UPDATE paper_test_state SET usdt=?, updated_ts=? WHERE username=?", (new_bal, ts_txt, username))
                else:
            ...
            conn.commit()
            return {"ok": True, "action": "SELL", "exchange_id": ex_db, "symbol": symbol, "exit_price": price, "pnl_net_usdt": pnl_net, "fee_total_usdt": fee_buy + fee_sell, "proceeds_usdt": proceeds}

        return {"ok": False, "err": "İşlem tipi geçersiz"}
...
def nav_html(is_admin: bool) -> str:
    items = [f'<a class="pill" href="/">{E_HOME} Panel</a>', f'<a class="pill" href="/test">{E_TST} Test</a>', f'<a class="pill" href="/charts">📈 Grafikler</a>']
    if not is_admin:
...
<head>
  ...
  <style>
    ...
    html,body{height:100%;background:var(--bg);color:var(--text);margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;}
    .topbar{
    ...
    .brand .dot{width:10px;height:10px;border-radius:50%;background:var(--blue);box-shadow:0 0 18px rgba(47,107,255,.55)}
    .tabs{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
    .tab{padding:8px 12px;border-radius:999px;border:1px solid var(--border);color:var(--muted);text-decoration:none;font-weight:700;font-size:13px}
    .tab.active{background:rgba(47,107,255,.12);border-color:rgba(47,107,255,.35);color:var(--text)}
    .rightbtns{display:flex;align-items:center;gap:10px}
    .btn{padding:8px 12px;border-radius:10px;border:1px solid var(--border);background:rgba(255,255,255,.03);color:var(--text);text-decoration:none;font-weight:800;font-size:13px}
    .btn.primary{background:rgba(47,107,255,.18);border-color:rgba(47,107,255,.45)}
    .wrap{max-width:1400px;margin:0 auto;padding:14px 14px 18px}
    .grid{display:grid;grid-template-columns:300px 1fr 320px;gap:12px;align-items:start}
    .panel{background:linear-gradient(180deg, rgba(255,255,255,.03), rgba(255,255,255,.01));border:1px solid var(--border);border-radius:14px;overflow:hidden}
    .panel .ph{padding:12px 12px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;gap:10px}
    .panel .ph .h{font-weight:900}
    .panel .pc{padding:12px}
    .input{width:100%;box-sizing:border-box;padding:10px 12px;border-radius:12px;border:1px solid var(--border);background:rgba(0,0,0,.15);color:var(--text);outline:none;font-weight:800}
    .muted{color:var(--muted)}
    .list{max-height:720px;overflow:auto}
    .row{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border-bottom:1px solid rgba(255,255,255,.06);cursor:pointer}
    .row:hover{background:rgba(255,255,255,.03)}
    .row.active{background:rgba(47,107,255,.12); border-left:3px solid var(--blue)}
    .sym{font-weight:900}
    .chg{font-weight:900}
    .chg.pos{color:var(--green)}
    .chg.neg{color:var(--red)}
    .price{font-weight:900}
    .mainTop{display:flex;align-items:flex-end;justify-content:space-between;gap:10px;flex-wrap:wrap;padding:12px 12px;border-bottom:1px solid var(--border)}
    .pair{font-size:18px;font-weight:1000}
    .bigPrice{font-size:22px;font-weight:1000}
    .stats{display:flex;gap:10px;flex-wrap:wrap}
    .stat{padding:8px 10px;border:1px solid rgba(255,255,255,.08);border-radius:12px;background:rgba(0,0,0,.12);min-width:120px}
    .stat .k{font-size:11px;color:var(--muted);font-weight:800}
    .stat .v{font-size:13px;font-weight:950;margin-top:2px}
    .subgrid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
    .tbl{width:100%;border-collapse:collapse}
    .tbl th,.tbl td{padding:8px 8px;border-bottom:1px solid rgba(255,255,255,.06);font-size:12px}
    .tbl th{text-align:left;color:var(--muted);font-weight:900}
    .sideBox{padding:12px}
    .sideTitle{font-weight:1000;margin-bottom:10px}
    .pill{display:inline-flex;gap:8px;align-items:center;padding:10px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.08);background:rgba(0,0,0,.12);font-weight:950}
    .buy{color:var(--green)}
    .sell{color:var(--red)}
    ...
    @media (max-width:1200px){
      ...
      .grid > .right{grid-column:1 / span 2}
    }
...
<div class="topbar">
  <div class="brand"><span class="dot"></span><span>AU Charts</span><span class="mini">Binance canlı</span></div>

  <div class="tabs">
    <a class="tab" href="/panel">Panel</a>
    <a class="tab active" href="/charts">Grafikler</a>
    <a class="tab" href="/test">Test</a>
  </div>
...
<div class="wrap">
  <div class="grid">
    ...
    <div class="panel">
      <div class="ph">
        <div class="h">Piyasalar</div>
        <div class="mini" id="srvClock">-</div>
      </div>
      <div class="pc">
        <input id="q" class="input" placeholder="Coin ara" autocomplete="off">
      </div>
    ...
    <div class="panel">
      <div class="mainTop">
        <div>
          <div class="pair" id="pair">BTCUSDT</div>
          <div class="bigPrice" id="last">-</div>
          <div class="mini" id="bidask">Bid -  Ask -</div>
        </div>
        <div class="stats">
          <div class="stat"><div class="k">24s değişim</div><div class="v" id="chg">-</div></div>
          <div class="stat"><div class="k">24s en yüksek</div><div class="v" id="hi">-</div></div>
          <div class="stat"><div class="k">24s en düşük</div><div class="v" id="lo">-</div></div>
          <div class="stat"><div class="k">24s ortalama</div><div class="v" id="avg">-</div></div>
          <div class="stat"><div class="k">Hacim</div><div class="v" id="vol">-</div></div>
        </div>
      </div>
      ...
      <div class="pc">
        ...
        <div class="subgrid">
          <div class="panel" style="border-radius:12px">
            <div class="ph"><div class="h">Derinlik</div><div class="mini">Top 15</div></div>
            <div class="pc">
              <table class="tbl">
                <thead><tr><th>Alış</th><th>Miktar</th><th>Satış</th><th>Miktar</th></tr></thead>
                <tbody id="ob"></tbody>
              </table>
            </div>
          </div>
          ...
          <div class="panel" style="border-radius:12px">
            <div class="ph"><div class="h">Son işlemler</div><div class="mini">Anlık</div></div>
            <div class="pc">
              <table class="tbl">
                <thead><tr><th>Zaman</th><th>Fiyat</th><th>Miktar</th></tr></thead>
                <tbody id="tr"></tbody>
              </table>
            </div>
          </div>
    ...
    <!-- RIGHT: ORDER PANEL (UI only) -->
    <div class="panel right">
      <div class="ph"><div class="h">İşlem</div><div class="mini">UI</div></div>
      <div class="sideBox">
        <div class="pill"><span>Fiyat</span><span style="margin-left:auto" id="uiPrice">-</span></div>
        <div class="sp"></div>
        <div class="mini">Bu sayfa sadece grafik ve piyasa verisi içindir. İşlem butonu yok.</div>
        <div class="sp"></div>
        <div class="pill buy">AL</div>
        <div class="sp"></div>
        <div class="pill sell">SAT</div>
      </div>
    </div>
...
(function(){
  const fmt = (n, d=6) => {
    ...
    return x.toLocaleString("tr-TR", {maximumFractionDigits: d});
  };

  const clockEl = document.getElementById("srvClock");
  function tickClock(){
    try{
      const d = new Date();
  ...
  const uiPriceEl = document.getElementById("uiPrice");
  ...
  const chart = LightweightCharts.createChart(document.getElementById("chart"), {
    ...
    height: 520,
    layout: { background: { type: 'solid', color: 'transparent' }, textColor: '#e9f0ff' },
    grid: { vertLines: { color: 'rgba(255,255,255,0.06)' }, horzLines: { color: 'rgba(255,255,255,0.06)' } },
    timeScale: { timeVisible: true, secondsVisible: false },
    rightPriceScale: { borderColor: 'rgba(255,255,255,0.12)' },
  });
  const candleSeries = chart.addCandlestickSeries({
    upColor: 'rgba(24,196,143,1)',
    downColor: 'rgba(255,91,107,1)',
    borderUpColor: 'rgba(24,196,143,1)',
    borderDownColor: 'rgba(255,91,107,1)',
    wickUpColor: 'rgba(24,196,143,1)',
    wickDownColor: 'rgba(255,91,107,1)',
  });
  ...
  function renderList(){
    ...
    const arr = q ? SYMBOLS.filter(s => s.includes(q)) : SYMBOLS;

    const frag = document.createDocumentFragment();
    arr.slice(0, 600).forEach(sym => {
      const div = document.createElement("div");
      div.className = "row" + (sym === active ? " active" : "");
      div.dataset.sym = sym;
      div.setAttribute("role","button");
      div.setAttribute("tabindex","0");
      div.innerHTML = `<div><div class="sym">${sym.replace("USDT","/USDT")}</div><div class="mini">Binance</div></div>
                       <div class="price" id="p_${sym}">-</div>`;
  ...
  // click/select delegation (daha stabil) + pointerdown (bazı tarayıcılarda click düşmeyebiliyor)
  ...
  listEl.addEventListener("click", pickFromEvent);
  listEl.addEventListener("pointerdown", pickFromEvent);
  listEl.addEventListener("mousedown", pickFromEvent);
  listEl.addEventListener("touchstart", pickFromEvent, {passive:false});

  // klavye ile hızlı seçim: Enter ilk sonucu seçer
  qEl.addEventListener("keydown", (ev)=>{
    if(ev.key === "Enter"){
      const first = listEl.querySelector(".row");
      if(first && first.dataset && first.dataset.sym){
        setActive(first.dataset.sym);
      }
    }
  });
  ...
  async function loadSymbols(){
    let j=null;
    try{
      j = await api("/api/binance/symbols");
      SYMBOLS = (j.symbols||[]).filter(s=>s.endsWith("USDT"));
    }catch(e){
      SYMBOLS = [];
    }
    if(!SYMBOLS || SYMBOLS.length===0){
      // fallback (API erişimi yoksa bile en azından liste boş kalmasın)
      SYMBOLS = ["BTCUSDT","ETHUSDT","SOLUSDT","XRPUSDT","BNBUSDT","TONUSDT","DOGEUSDT","ADAUSDT"];
    }
    renderList();
  }

  async function setActive(sym){
    active = sym;
    pairEl.textContent = sym;
    renderList();
    // seçili olanı görünür al
    try{ const el=listEl.querySelector(`.row[data-sym="${sym}"]`); if(el) el.scrollIntoView({block:'nearest'});}catch(e){}
    await loadCandles();
  ...
  async function loadCandles(){
    const j = await api(`/api/binance/klines?symbol=${encodeURIComponent(active)}&interval=1m&limit=600`);
    candleSeries.setData(j.candles||[]);
    chart.timeScale().fitContent();
  }

  async function refreshSummary(){
    const j = await api(`/api/binance/summary?symbol=${encodeURIComponent(active)}`);
    lastEl.textContent = fmt(j.last, 6);
    uiPriceEl.textContent = fmt(j.last, 6);
    bidaskEl.textContent = `Bid ${fmt(j.bid,6)}   Ask ${fmt(j.ask,6)}`;

    const ch = Number(j.changePercent||0);
    const s = (ch>=0) ? `+${fmt(ch,2)}%` : `${fmt(ch,2)}%`;
    chgEl.textContent = s;

    hiEl.textContent = fmt(j.high,6);
    loEl.textContent = fmt(j.low,6);
    avgEl.textContent = fmt(j.avg,6);
    volEl.textContent = fmt(j.quoteVolume,2);
  }

  async function refreshDepth(){
    const j = await api(`/api/binance/depth?symbol=${encodeURIComponent(active)}&limit=15`);
    const bids = j.bids||[];
    const asks = j.asks||[];
    ...
    for(let i=0;i<n;i++){
      ...
      const a = asks[i] || ["",""];
      html += `<tr>
        <td class="buy">${fmt(b[0],6)}</td><td>${fmt(b[1],4)}</td>
        <td class="sell">${fmt(a[0],6)}</td><td>${fmt(a[1],4)}</td>
      </tr>`;
    }
    document.getElementById("ob").innerHTML = html || `<tr><td colspan="4" class="muted">Veri yok</td></tr>`;
  }

  async function refreshTrades(){
    const j = await api(`/api/binance/trades?symbol=${encodeURIComponent(active)}&limit=18`);
    const arr = j.trades||[];
    let html="";
    arr.forEach(t=>{
      const d = new Date(t.time);
      const tm = d.toLocaleTimeString("tr-TR");
      html += `<tr>
        <td class="muted">${tm}</td>
        <td class="${t.isBuyerMaker ? 'sell':'buy'}">${fmt(t.price,6)}</td>
        <td>${fmt(t.qty,4)}</td>
      </tr>`;
    });
    document.getElementById("tr").innerHTML = html || `<tr><td colspan="3" class="muted">Veri yok</td></tr>`;
  }

  // WebSocket: trade tick every moment
  function startWS(){
    try{ if(ws){ ws.close(); ws=null; } }catch(e){}
    const s = active.toLowerCase();
    ws = new WebSocket(`wss://stream.binance.com:9443/ws/${s}@trade`);
    let lastBar = null;

    ws.onmessage = (ev)=>{
      try{
        const j = JSON.parse(ev.data||"{}");
        const price = Number(j.p);
        const ts = Number(j.T);
        if(!isFinite(price) || !isFinite(ts)) return;

        // update last price UI
        lastEl.textContent = fmt(price,6);
        uiPriceEl.textContent = fmt(price,6);

        // update current candle (1m)
        const t = Math.floor(ts/60000)*60; // seconds
        if(!lastBar || lastBar.time !== t){
    ...
    ws.onclose = ()=>{
      // auto reconnect
      setTimeout(()=>{ if(active && active.length>0) startWS(); }, 1500);
    };
  }

  // list price mini updates (light)
  async function refreshListPrices(){
    // 24hr mini ticker for all USDT is heavy; we fetch one by one only visible rows
    const rows = Array.from(document.querySelectorAll(".row"));
    const vis = rows.slice(0, 60); // keep light
    for(const r of vis){
      const sym = r.dataset.sym;
      try{
        const j = await api(`/api/binance/mini?symbol=${encodeURIComponent(sym)}`);
        const el = document.getElementById(`p_${sym}`);
        if(el) el.textContent = fmt(j.last,6);
      }catch(e){}
    }
  }
  ...
  // periodic refresh panels (REST)
  setInterval(()=>{ refreshSummary().catch(()=>{}); }, 1500);
  setInterval(()=>{ refreshDepth().catch(()=>{}); refreshTrades().catch(()=>{}); }, 2000);
  setInterval(()=>{ refreshListPrices().catch(()=>{}); }, 2500);

  // init
  loadSymbols()
    .then(()=>setActive("BTCUSDT"))
    .catch(()=>{});
})();
</script>
...
""")
    ...
    return tpl.safe_substitute(USERNAME=html_escape(username))
...
def test_reset():
    ...
    try:
        with sqlite3.connect(DB_PATH) as con:
            cur = con.cursor()
            for t in ["paper_test_trades", "paper_test_positions", "paper_test_state", "paper_test_autorules"]:
                try:
                    cur.execute(f"DELETE FROM {t} WHERE username=?", (username,))
                except Exception:
                    pass
            # başlangıç state
            try:
...
def _render_user_dashboard(test_view: bool = False):
    ...
    def stat_box(title: str, s: Dict[str, Any]) -> str:
        # paper stats returns {count,pnl,ratio}; live stats returns {count,pnl_try,pct}
        pnl_try = float(s.get('pnl_try', s.get('pnl', 0.0)) or 0.0)
        pct = float(s.get('pct', s.get('ratio', 0.0)) or 0.0)
        cnt = int(s.get('count', 0) or 0)
        sign = E_ON if pnl_try >= 0 else E_OFF
        return f"""
        <div class="box">
          <div class="k">{title}</div>
          <div class="v">{sign} {cnt} işlem</div>
    ...
    try:
        ...
        if positions:
            for p in positions:
                pid = int(p.get("id") or 0)
                if test_view:
                    sell_btn = f'''<form method="post" action="/manual/enqueue" style="margin:0">
                      <input type="hidden" name="exchange_id" value="{ex_name or 'OKX'}">
                      <input type="hidden" name="symbol" value="{sym}">
                      <input type="hidden" name="action" value="SELL">
                      <input type="hidden" name="usdt" value="0">
                      <input type="hidden" name="all_in" value="1">
                      <button class="btn bad" type="submit">{E_SELL} SELL</button>
                    </form>'''
                else:
                    sell_btn = f'<form method="post" action="/position/{pid}/enqueue_sell" style="margin:0"><button class="btn bad" type="submit">{E_SELL} SELL</button></form>'
                ex_name = safe_str(p.get("exchange_id") or p.get("exchange") or "").upper()
                sym = safe_str(p.get("symbol") or "").upper()
                qty = float(p.get("qty") or 0.0)
                entry = float(p.get("entry_price") or p.get("entry_px") or 0.0)
                entry_usdt = float(p.get("entry_usdt") or p.get("invested_usdt") or 0.0)
    ...
    try:
        ...
        if rules:
            for r in rules:
                ...
                usdt = float(r.get("usdt_amount") or 0.0)
...
def manual_enqueue():
    ...
    # Panel context izolasyon
    ctx = _panel_ctx()

    # Manuel işlem ekranında GERÇEK emir kapalı (sadece TEST panelinde paper-trade)
    if ctx != "test":
        try:
            set_last_msg(username, f"{E_STOP} Manuel işlemler sadece Test panelinde kullanılabilir")
        except Exception:
            pass
        return redirect_back('/')
    ...
    # DRY/LIVE seçimi
    # Manuel ekran: sadece TEST panelinde paper-trade çalışır (GERÇEK kapalı)
    # UI tarafında TEST seçili gelir; backend de güvenlik için burada zorlar.

    # miktar
    all_in = safe_str(request.form.get("all_in") or "").strip().lower() in ("1", "true", "on", "yes")
    try:
        usdt = float(safe_str(request.form.get("usdt") or "0") or 0.0)
    except Exception:
        usdt = 0.0

    if action == "BUY":
        if (not all_in) and usdt <= 0:
            try:
                set_last_msg(username, f"{E_X} BUY başarısız: Geçersiz tutar")
            except Exception:
                pass
            return redirect_back("/test")
        res = paper_test_exec_order(username, "BUY", exchange_id, symbol, usdt, all_in)
        if not (res or {}).get("ok"):
            err = safe_str((res or {}).get("err") or "BUY başarısız")
            try:
                set_last_msg(username, f"{E_X} BUY başarısız: {err}")
            except Exception:
                pass
            return redirect_back("/test")
        try:
            qty = float((res or {}).get("qty") or 0.0)
            px = float((res or {}).get("price") or 0.0)
            fee = float((res or {}).get("fee_buy") or 0.0)
            set_last_msg(username, f"{E_OK} TEST BUY • {symbol} • {qty:.8f} @ {px:.6f} • Fee {fee:.6f}")
        except Exception:
            try:
                set_last_msg(username, f"{E_OK} TEST BUY • {symbol}")
            except Exception:
                pass
        return redirect_back("/test")

    if action == "SELL":
        res = paper_test_exec_order(username, "SELL", exchange_id, symbol, 0.0, False)
        if not (res or {}).get("ok"):
            err = safe_str((res or {}).get("err") or "SELL başarısız")
            try:
            ...
            return redirect_back("/test")
        try:
            pnl = float((res or {}).get("pnl") or 0.0)
            fee = float((res or {}).get("fee_sell") or 0.0)
            set_last_msg(username, f"{E_OK} TEST SELL • {symbol} • Net PnL {pnl:.6f} • Fee {fee:.6f}")
        except Exception:
            try:
                set_last_msg(username, f"{E_OK} TEST SELL • {symbol}")
            except Exception:
                pass
        return redirect_back("/test")
...
def enqueue_sell(pid: int):
    ...
    # TEST panelinde SELL: paper_test_positions kapat
    if _panel_ctx() == "test":
        conn = db()
        try:
            row = conn.execute(
                "SELECT * FROM paper_test_positions WHERE id=? AND username=?",
                (int(pid), username),
            ).fetchone()
        finally:
            try:
        ...
        if not row:
            return redirect_back("/test")
        pr = dict(row)
        exx = safe_str(pr.get("exchange_id") or pr.get("exchange") or "OKX").upper() or "OKX"
        sym = safe_str(pr.get("symbol") or "")
        res = paper_test_exec_order(username, "SELL", exx, sym, 0.0, all_in=False)
        if (res or {}).get("ok"):
            try:
                set_last_msg(username, f"{E_OK} TEST SELL • {canon_symbol(sym)}")
            except Exception:
                pass
        else:
            try:
                set_last_msg(username, f"{E_X} TEST SELL hata • {safe_str((res or {}).get('err') or 'Bilinmiyor')}")
            except Exception:
                pass
        return redirect_back("/test")
...
def auto_create_form():
    ...
    ctx = _panel_ctx()
    if ctx == "test":
        # TEST: auto kuralları paper_test_autorules tablosuna yazılır (izole)
        exchange_id = safe_str(request.form.get("exchange_id") or "").strip().upper()
        ...
        try:
            conn.execute(
                "INSERT INTO paper_test_autorules(username, exchange_id, symbol, usdt_amount, use_all_balance, enabled, created_at) VALUES (?,?,?,?,?,?,?)",
                (username, exchange_id, canon_symbol(symbol), float(usdt), 1 if all_in else 0, 1, now_ts())
            )
            conn.commit()
        finally:
            try:
                ...
                pass
        try:
            set_last_msg(username, f"{E_OK} TEST AUTO kural eklendi • {canon_symbol(symbol)}")
        except Exception:
            pass
        return redirect_back("/test")
...
def auto_toggle_form(rid: int):
    ...
    try:
        if ctx == "test":
            row = conn.execute(
                "SELECT enabled FROM paper_test_autorules WHERE id=? AND username=?",
                (int(rid), username),
            ).fetchone()
            if row is not None:
                curv = int((row[0] or 0))
                ...
                conn.execute(
                    "UPDATE paper_test_autorules SET enabled=? WHERE id=? AND username=?",
                    (newv, int(rid), username),
                )
                conn.commit()
            return redirect_back("/test")

        row = conn.execute(
            "SELECT enabled FROM auto_rules WHERE id=? AND username=?",
            (int(rid), username),
        ).fetchone()
        if row is not None:
            curv = int((row[0] or 0))
            newv = 0 if curv == 1 else 1
            conn.execute(
                "UPDATE auto_rules SET enabled=? WHERE id=? AND username=?",
                (newv, int(rid), username),
            )
            conn.commit()
        return redirect_back("/")
...
def auto_delete_form(rid: int):
    ...
    try:
        if ctx == "test":
            conn.execute("DELETE FROM paper_test_autorules WHERE id=? AND username=?", (int(rid), username))
            conn.commit()
            return redirect_back("/test")

        conn.execute("DELETE FROM auto_rules WHERE id=? AND username=?", (int(rid), username))
        conn.commit()
        return redirect_back("/")
    finally:
        try:
...
def auto_update_form(rid: int):
    ...
    if not exchange_id or not symbol:
        return redirect_back("/test" if ctx=="test" else "/")

    conn = db()
    try:
        if ctx == "test":
            conn.execute(
                "UPDATE paper_test_autorules SET exchange_id=?, symbol=?, usdt_amount=?, use_all_balance=? WHERE id=? AND username=?",
                (exchange_id, canon_symbol(symbol), float(usdt), 1 if all_in else 0, int(rid), username),
            )
            conn.commit()
            return redirect_back("/test")

        conn.execute(
            "UPDATE auto_rules SET exchange_id=?, symbol=?, usdt=?, enabled=enabled WHERE id=? AND username=?",
            (exchange_id, canon_symbol(symbol), float(usdt if usdt>0 else 0.0), int(rid), username),
        )
        conn.commit()
        return redirect_back("/")
    finally:
...
if __name__ == "__main__":
    ...
    try:
        ...
        pass
    try:
        threading.Thread(target=_paper_auto_loop, daemon=True).start()
    except Exception:
        pass
